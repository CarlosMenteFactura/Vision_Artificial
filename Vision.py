...
class Vision_1:
    ...
    def Trigger(self,elemento,accion):
        try:
            ...
            print("El arduino no esta conectado o esta en otra red")
    ...
    def main(self,BroccoliAcceptedSizes) :
        ...
        dc = DepthCamera(self.serial_number[self.puerto_camara])  # colocar en mejor lugar
        #dc = DepthCamera()
        camara = cv2.VideoCapture(self.puerto_camara)
        camara, h, w = self.CamaraSettings(camara)
        ...
        """ Se ocupa para la comunicasion"""
        #TCP_toggle_onoff = self.TCPToggleOnOff()
        #self.TestComunicacionModbusTPCIP("no")
        """------------------------------"""
        ...
        if camara.isOpened() :
            ...
            while 1:
                ...
                ret, depth_frame, frame_2, accel = dc.get_frame()
                ...
                cv2.putText(frame, f"Brocolis Cortados:{BroccolisCut}", (w - 280, h - 38),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (200, 230, 124), 2)
                ...
                for j in range(0, objetosEnVideo) :
                    ...
                    if tamano_RealX >= BroccoliSizeMin and tamano_RealX <= BroccoliSizeMax :
                        ...
                        ban = True
                        #bro[j]=[round(tamano_RealX),tamano_RealY]
                    else :
                ...
                if suma > 0:
                   ...
                   # tamano_entero=bro[j][0]
                   # print(f"++++{tamano_RealX}---")
                    if ban:
                        #print(f"++++{tamano_RealX}---{bro[pos][0]}")
                        #print("1")
                        #print(ban)
                        self.ConexionBaseDatos.Incremento(dick[tamano_RealX], 3)
                        self.ConexionBaseDatos.Incremento("CantidadCortada", 3)
                        PilaTiempo.append(distancia)
                        BroccolisCut+=1
                    else:
                        if tamano_RealX<BroccoliSizeMin:
                            self.ConexionBaseDatos.Incremento(dick["menor"], 3)
                            self.ConexionBaseDatos.Incremento("CantidadPorDebajo", 3)
                        else:
                            self.ConexionBaseDatos.Incremento(dick["mayor"], 3) 
                            self.ConexionBaseDatos.Incremento("CantidadPorEncima", 3)
                    self.ConexionBaseDatos.Incremento("CantidadObservada", 3)
                bAnt = bAct.copy()
