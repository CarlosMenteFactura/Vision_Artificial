...
class Vision_2:
    def __init__(self, puerto_camara=1,datos_video="Camara1"):
        self.serial_number={0:"241122305779",2:"234322304889"}
        self.ConexionBaseDatos=Creacion(Datos)
        self.puerto_camara=puerto_camara
        self.datos_video=datos_video
    ...
    def main(self,BroccoliAcceptedSizes) :
        ...
        Archivo.write('*********************************************************************************\n')
        Videoresult = cv2.VideoWriter(r'V2.avi', cv2.VideoWriter_fourcc(*'MJPG'), 30, (int(w), int(h)))
        if camara.isOpened() :
            a=time.time()
            while 1:
               ...
               # vel, accel_ant, sumA, tiempo = self.Velocidad(fondo, accel[0], accel_ant, sumaA, tiempo)
                ...
                if suma > 0 :
                    ...
                    Archivo.write(f"Brocoli: {BroccolisCounter}  tamaño x:{bro[pos][0]:.2f}  tamaño Y: {bro[pos][1]:.2f}\n")
                    tamano_entero=round(bro[pos][1])
                    print(tamano_entero)
                    if(tamano_entero>=14 and tamano_entero<=30):
                        self.ConexionBaseDatos.Incremento(dick[tamano_entero], 3)
                    ...
                    #self.Trigger(6, 1)
                    #self.Trigger(6, 0)
                    print("sensor 6 activado")
                bAnt = bAct.copy()
                
                PilaTiempo = self.corte_con_pilas(vel, PilaTiempo, distancia)
            
                # Window PopUp
                cv2.namedWindow(self.datos_video, cv2.WINDOW_NORMAL)
                cv2.imshow(self.datos_video, frame)
